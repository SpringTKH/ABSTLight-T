...
class Config:
    ...
    REWARD_THROUGHPUT_WEIGHT = (
        1.0  # Destination arrival events per step (positive signal)
    )
    REWARD_PRESSURE_WEIGHT = -0.5  # Queue buildup penalty (normalized & capped)
    REWARD_INSERTION_PENALTY_WEIGHT = (
        -1.0
    )  # Pending vehicles globally (normalized & clipped)
    REWARD_PHASE_CHANGE = -1.0  # Phase transition cost (reduces flickering)
